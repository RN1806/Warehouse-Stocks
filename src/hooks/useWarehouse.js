...
// New delivery/sample forms are always created as a Draft — let admins know
// right away so they can review it instead of finding out later.
...
// Notify every admin (sales_reps.role = 'admin') that a new draft delivery
// form was saved, so they can review/approve it.
async function notifyAdminsNewDraft(delivery, items) {
try {
const { data: admins } = await supabase.from('sales_reps').select('email').eq('role', 'admin')
if (!admins || admins.length === 0) return
const who = delivery.customer_name || (delivery.delivery_type === 'lab' ? 'Lab use' : 'Customer delivery')
...
type: 'draft_delivery',
title: `New draft sample form: ${delivery.form_number}`,
body: `${who} — ${itemCount} product(s) — by ${delivery.sales_rep_name || 'unknown'}. Awaiting review.`,
}))
if (rows.length > 0) {
await supabase.from('notifications').insert(rows)
}
} catch {
// Never let a notification failure block saving the delivery itself.
}
}
...
// Update where a product is kept (storage location). Used by the Givaudan
// collections UI, but works for any product.
export async function updateProductStorageLocation(productId, storageLocation) {
const { error } = await supabase.from('products')
.update({ storage_location: storageLocation?.trim() || null }).eq('id', productId)
if (error) throw error
}
