...
import { useProducts, createDelivery, updateDelivery, getDelivery, visibleProductsFor, useBrandOwners, approvalCheck, createSampleRequest, useIncomingRequests, hasApproval } from '../hooks/useWarehouse'
...
function blankForm(profile, today) {
return {
delivery_date: today,
...
// Pass editingId to load & edit an existing delivery instead of creating a new one.
export default function NewDeliveryPage({ onSaved, onBack, editingId }) {
...
const [form, setForm] = useState(() => blankForm(profile, today))
...
if (!editingId) return
let cancelled = false
getDelivery(editingId).then(d => {
if (cancelled) return
setEditingOwnerId(d.sales_rep_id)
const { id, form_number, delivery_items, created_at, ...rest } = d
setForm(rest)
const sorted = [...(delivery_items || [])].sort((a,b) => a.item_order - b.item_order)
setItems(sorted.length > 0
? sorted.map(it => ({
product_name: it.product_name || '', amount: it.amount ?? '', unit: it.unit || 'g',
packaging_description: it.packaging_description || '', lot_no: it.lot_no || '', remark: it.remark || '',
}))
: [{...EMPTY_ITEM},{...EMPTY_ITEM},{...EMPTY_ITEM}])
setLoadingEdit(false)
}).catch(() => { if (!cancelled) { setNotFound(true); setLoadingEdit(false) } })
return () => { cancelled = true }
}, [editingId])

const canEditThis = !editingId || isAdmin || editingOwnerId === profile?.id
...
try {
if (editingId) {
await updateDelivery(editingId, form, filled)
} else {
...
if (loadingEdit) return <div className="pt-20"><Spinner /></div>

if (notFound) return (
<div className="px-4 pt-20 text-center">
<p className="text-sm text-gray-500 mb-4">This delivery form couldn't be found.</p>
...
<div className="px-4 pt-20 text-center">
<p className="text-sm text-gray-500 mb-4">Only the form's creator or an admin can edit this delivery.</p>
<button onClick={onBack} className="text-sm text-blue-700">← Back</button>
</div>
)
...
<h1 className="text-white text-lg font-semibold">{editingId ? 'Edit Delivery Form' : 'New Delivery Form'}</h1>
...
{saving ? 'Saving…' : editingId ? '💾 Save Changes' : '💾 Save Delivery Form'}
</button>
</div>
</form>
