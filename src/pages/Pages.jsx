...
setPw1(''); setPw2(''); setPwErr(''); setPwDone(false)
setChangingPw(true)
}
...
<h2 className="text-base font-semibold text-slate-900 display">Change password</h2>
{pwDone ? (
<>
<p className="text-sm text-green-700 bg-green-50 rounded-lg px-3 py-2">✓ Password updated successfully.</p>
<button onClick={() => setChangingPw(false)} className="w-full btn-primary rounded-xl py-3 text-sm">Done</button>
</>
) : (
<>
<div>
...
<div className="flex gap-2">
<button onClick={() => setChangingPw(false)} className="flex-1 border border-gray-200 rounded-xl py-3 text-sm text-gray-600">Cancel</button>
<button onClick={handleChangePassword} disabled={pwSaving} className="flex-1 btn-primary rounded-xl py-3 text-sm">{pwSaving ? 'Saving…' : 'Save'}</button>
</div>
</>
)}
</div>
...
<button onClick={startChangePw} className="w-full border border-gray-200 text-gray-700 rounded-xl py-3 text-sm font-medium hover:bg-gray-50 mb-3">🔒 Change password</button>
