...
useCollections, addCollection, addProductToCollection, updateProductExpiry, updateProductStorageLocation, deleteCollection } from '../hooks/useWarehouse'
...
const [collProd, setCollProd] = useState({ name:'', expiry:'', unit:'g', location:'' })
const [collBusy, setCollBusy] = useState(false)
const [editingLocation, setEditingLocation] = useState(null) // product being given a storage location
const [locationVal, setLocationVal] = useState('')
const [locationSaving, setLocationSaving] = useState(false)

// Quick lookup: collection id -> collection name, used anywhere in the
// product list (including search results) so a product's collection shows
// up without needing to open the Givaudan Collections panel.
const collectionNameById = {}
collections.forEach(c => { collectionNameById[c.id] = c.name })
...
setCollBusy(true)
try {
await addProductToCollection({ name: collProd.name, collectionId: addToColl, expiryDate: collProd.expiry, defaultUnit: collProd.unit, storageLocation: collProd.location })
setCollProd({ name:'', expiry:'', unit:'g', location:'' }); setAddToColl(null); refetch()
} catch (e) { alert(e.message) } finally { setCollBusy(false) }
}
...
try {
await updateProductStorageLocation(editingLocation.id, locationVal)
await refetch()
...
{p.storage_location && <span className="text-blue-600"> · 📍 {p.storage_location}</span>}
</p>
</div>
{isAdmin && (
<button onClick={() => { setEditingLocation(p); setLocationVal(p.storage_location || '') }}
className="w-6 h-6 rounded text-slate-300 hover:text-blue-500" title="Set keeping place">📍</button>
)}
{isAdmin && (
...
<div>
<label className="block text-[11px] text-slate-500 mb-0.5">Keeping place / Storage location</label>
<input value={collProd.location} onChange={e => setCollProd(s => ({ ...s, location: e.target.value }))}
...
<button onClick={() => { setAddToColl(c.id); setCollProd({ name:'', expiry:'', unit:'g', location:'' }) }}
...
{p.collection_id && collectionNameById[p.collection_id] && (
<p className="text-[11px] text-purple-600 mt-0.5">
📂 {collectionNameById[p.collection_id]}
{p.storage_location && <span className="text-blue-600"> · 📍 {p.storage_location}</span>}
</p>
)}
</div>
...
{/* Set storage location modal (admin) */}
{editingLocation && (
<div className="fixed inset-0 z-50 flex items-center justify-center px-6">
<div className="absolute inset-0 bg-black/40" onClick={() => setEditingLocation(null)} />
<div className="relative bg-white w-full max-w-sm rounded-2xl p-5 shadow-xl">
<h2 className="text-base font-semibold text-gray-900 mb-1">Keeping place</h2>
<p className="text-xs text-gray-400 mb-4">{editingLocation.name}</p>
<input value={locationVal} onChange={e => setLocationVal(e.target.value)}
placeholder="e.g. Shelf A3, Cold Room 2" className={inputCls} autoFocus />
<div className="flex gap-2 mt-4">
<button onClick={() => setEditingLocation(null)}
className="flex-1 border border-gray-200 rounded-xl py-2.5 text-sm text-gray-600">Cancel</button>
<button disabled={locationSaving}
